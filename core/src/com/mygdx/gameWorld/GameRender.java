...
public class GameRender {
	...
	private Forms cForm;
	private Forms nForm;
	public GameRender(GameWorld world,MyGdxGame game) {
		this.world=world;
    ...
    public void render() {
    	if(cForm==null) {
    	cForm=world.getForm(0);
    	nForm=world.getForm(1);	
        ...
        int[][] curForm=cForm.generateForm();
        int[][] nexForm=nForm.generateForm();
        ...
        for(int x=0;x<curForm.length;x++) {
        	for(int y=0;y<curForm.length;y++) {
        		if(curForm[x][y]>0)
        			if(cForm!=Forms.L_FORM)
        		batch.draw(tex[5],600+x*30, +500+y*30);else batch.draw(tex[5],600+x*30, +500+y*30-30);
        ...
        for(int x=0;x<nexForm.length;x++) {
        	for(int y=0;y<nexForm.length;y++) {
        		if(nexForm[x][y]>0)
        			if(nForm!=Forms.L_FORM)
        		batch.draw(tex[5],600+x*30, +250+y*30);else batch.draw(tex[5],600+x*30, +250+y*30-30);
        		
        	}
        	
        }
        
        font.draw(batch, "Score: "+Integer.toString(world.getScore()), 70, 700);
        if(points!=null)for(Coord c :points) {batch.draw(tex[world.getColor()-1],280+c.x*30, c.y*30);}	
